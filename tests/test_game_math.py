...
def test_blackjack_fair_payouts_constants() -> None:
    """The fair (no-edge) payouts are the published industry numbers:
    2x on a regular win, 2.5x on a natural blackjack."""
    assert game_math.BLACKJACK_WIN_FAIR == 2.0
    assert game_math.BLACKJACK_NATURAL_FAIR == 2.5


@pytest.mark.parametrize(
    "edge,expected_win,expected_natural",
    [
        # 0% edge -> exactly the fair payouts
        (0.0, 2.0, 2.5),
        # 1% edge (originals)
        (0.01, 1.98, 2.475),
        # 3% edge (blackjack default)
        (0.03, 1.94, 2.425),
        # 5% edge (hypothetical tighter house)
        (0.05, 1.9, 2.375),
    ],
)
def test_blackjack_payout_multipliers(
    edge: float, expected_win: float, expected_natural: float
) -> None:
    """Regular win pays ``2 * (1 - edge)``; natural pays ``2.5 * (1 - edge)``."""
    assert math.isclose(
        game_math.fair_blackjack_win_multiplier(edge),
        expected_win,
        abs_tol=1e-9,
    )
    assert math.isclose(
        game_math.fair_blackjack_natural_multiplier(edge),
        expected_natural,
        abs_tol=1e-9,
    )


def test_blackjack_payouts_monotone_in_edge() -> None:
    """Higher edge => smaller multiplier (always true)."""
    edges = [0.0, 0.005, 0.01, 0.02, 0.03, 0.05, 0.10]
    win_mults = [game_math.fair_blackjack_win_multiplier(e) for e in edges]
    nat_mults = [game_math.fair_blackjack_natural_multiplier(e) for e in edges]
    assert win_mults == sorted(win_mults, reverse=True)
    assert nat_mults == sorted(nat_mults, reverse=True)


def test_blackjack_natural_always_pays_more_than_regular() -> None:
    """A natural pays 3:2 vs 1:1 for a regular win -- this invariant
    must hold at every edge or the bonus has been lost."""
    for edge in [0.0, 0.005, 0.01, 0.02, 0.03, 0.05]:
        win = game_math.fair_blackjack_win_multiplier(edge)
        nat = game_math.fair_blackjack_natural_multiplier(edge)
        assert nat > win, (
            f"edge={edge}: natural {nat} should beat regular win {win}"
        )


# ---------------------------------------------------------------------
# effective_rakeback_edge (Phase 1c, audit M14)
# ---------------------------------------------------------------------


@pytest.mark.parametrize(
    "declared,jackpot,expected",
    [
        # Originals at 1% with default 0.2% jackpot -> house keeps 0.8%
        (0.01, 0.002, 0.008),
        # Blackjack at 3% with default 0.2% jackpot -> house keeps 2.8%
        (0.03, 0.002, 0.028),
        # Slots at 4% with default 0.2% jackpot -> house keeps 3.8%
        (0.04, 0.002, 0.038),
        # No jackpot configured -> rakeback uses the full declared edge
        (0.01, 0.0, 0.01),
        # PvP at 0.5% with default 0.2% jackpot -> house keeps 0.3%
        (0.005, 0.002, 0.003),
    ],
)
def test_effective_rakeback_edge_known_values(
    declared: float, jackpot: float, expected: float
) -> None:
    result = game_math.effective_rakeback_edge(declared, jackpot)
    assert math.isclose(result, expected, abs_tol=1e-9)


def test_effective_rakeback_edge_clamped_to_zero() -> None:
    """If jackpot rate exceeds declared edge, rakeback edge floors at 0.

    The previous (un-clamped) formula would have returned a negative
    value, which when multiplied by ``vip_rakeback_pct`` would have
    silently *deducted* money from the user's rakeback balance on
    every bet.
    """
    assert game_math.effective_rakeback_edge(0.005, 0.01) == 0.0
    assert game_math.effective_rakeback_edge(0.0, 0.002) == 0.0
    # Edge case: equal -> zero
    assert game_math.effective_rakeback_edge(0.002, 0.002) == 0.0


def test_effective_rakeback_edge_default_jackpot_is_zero() -> None:
    """If the caller forgets to pass ``jackpot_rate`` we get back the
    full declared edge -- conservative, no surprises."""
    for edge in [0.0, 0.005, 0.01, 0.03, 0.05]:
        assert game_math.effective_rakeback_edge(edge) == edge


def test_effective_rakeback_edge_does_not_double_charge_jackpot() -> None:
    """Money-conservation invariant: the player's expected loss per
    $1 wagered equals ``declared_edge`` (which gets split between the
    house and the jackpot pool). Rakeback must rebate ONLY the
    house's portion -- otherwise the bot effectively pays the jackpot
    cut twice (once into the pool, once back via rakeback).
    """
    declared = 0.01
    jackpot = 0.002
    # House actually keeps:
    house_actual = declared - jackpot
    # Effective rakeback edge:
    rakeback_edge = game_math.effective_rakeback_edge(declared, jackpot)
    # The two must agree to the cent on $1k:
    assert math.isclose(rakeback_edge, house_actual, abs_tol=1e-9)
    # 100% rakeback at this rate would refund exactly what the
    # house took (NOT what the jackpot took):
    bet = 1_000.0
    full_rakeback = bet * rakeback_edge
    expected_house_take = bet * house_actual
    assert math.isclose(full_rakeback, expected_house_take, abs_tol=1e-9)
